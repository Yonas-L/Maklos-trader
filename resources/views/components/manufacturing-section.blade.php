<section id="manufacturing"
    ...
    <div class="relative z-10 mx-auto max-w-7xl px-6 js-manufacturing-pin">
        ...
        <div
            class="js-manufacturing-track relative h-[48vh] md:h-[54vh] lg:h-[58vh] flex overflow-visible will-change-transform">               
            ...
            <article
                ...
                <div class="w-full lg:w-1/2 max-w-xl">
                    <div class="inline-flex items-center gap-3 mb-6">
                        <div
                            class="js-manufacturing-step-number inline-flex items-center justify-center w-12 h-12 rounded-2xl bg-white/15 text-white text-lg font-semibold shadow-lg ring-1 ring-white/30">                                         {{ $step['step_number'] }}
                        </div>
                        <span
                            class="js-manufacturing-badge inline-flex items-center px-3 py-1 rounded-full border border-white/30 bg-white/10 text-xs font-medium uppercase tracking-[0.16em] text-white/80">                                        {{ $step['badge'] }}
                        </span>
                    </div>

                    <h3 class="js-manufacturing-step-title text-4xl md:text-6xl font-bold text-white mb-4">                                                             {{ $step['title'] }}
                    </h3>

                    <p class="js-manufacturing-step-description text-lg md:text-xl text-white/90 leading-relaxed mb-5">                                                 {{ $step['description'] }}
                    </p>

                    <div class="space-y-2 text-base text-white/85">
                        @foreach ($step['features'] as $feature)
                        <p class="js-manufacturing-feature flex items-center gap-2">                                                                                        <span class="inline-flex h-1.5 w-1.5 rounded-full bg-white/80"></span>                                                                          {{ $feature }}
                        </p>
                        @endforeach
                    </div>
                </div>

                <div class="w-full lg:w-1/2 h-[260px] sm:h-[320px] md:h-[380px] lg:h-[460px] xl:h-[520px]">                                                         <div class="js-manufacturing-image relative h-full w-full"></div>                                                                           </div>
