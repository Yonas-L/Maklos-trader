...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="bg-white font-sans text-charcoal antialiased">
        ...
        @php
            $gallery = collect($product['gallery_urls'] ?? $product['gallery'] ?? [])->filter()->values();
        @endphp
        ...
        <main class="pt-24">
            <section class="relative overflow-hidden bg-gradient-to-br from-maklos-50/70 via-white to-maklos-100/40">
                ...
                <div class="mx-auto grid max-w-7xl gap-16 px-6 py-28 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,1fr)] lg:items-center js-product-hero" data-animate="hero">
                    <div class="space-y-8">
                        <div class="inline-flex items-center gap-3 rounded-full border border-maklos-200/60 bg-white/70 px-4 py-2 text-xs font-semibold uppercase tracking-[0.3em] text-maklos-600">
                            {{ $product['category'] ?? 'Maklos Product' }}
                        </div>
                        <h1 class="font-display text-4xl font-semibold text-charcoal sm:text-5xl lg:text-6xl">
                            {{ $product['name'] ?? 'Maklos Product' }}
                        </h1>
                        <p class="max-w-2xl text-lg text-charcoal/70 sm:text-xl">
                            {{ $product['description'] ?? $product['excerpt'] ?? 'Discover this Maklos innovation crafted with eucalyptus-powered science.' }}
                        </p>
                        <div class="flex flex-wrap items-center gap-4 text-sm font-semibold">
                            <a href="mailto:{{ config('app.contact_email', 'info@maklostrader.com') }}" class="inline-flex items-center gap-2 rounded-full bg-maklos-500 px-6 py-3 text-white shadow-lg shadow-maklos-500/30 transition hover:-translate-y-0.5 hover:bg-maklos-400">
                                Request wholesale
                            </a>
                            <a href="https://wa.me/{{ config('app.whatsapp_number', '251000000000') }}" class="inline-flex items-center gap-2 rounded-full border border-maklos-200 px-6 py-3 text-maklos-700/90 transition hover:border-maklos-400 hover:text-maklos-500">
                                Chat on WhatsApp
                            </a>
                        </div>
                    </div>
                    ...
                    <div class="relative flex justify-center lg:justify-end">
                        <div class="relative h-[480px] w-[360px] overflow-hidden rounded-[3rem] border border-white/60 bg-white shadow-2xl shadow-maklos-200/40 js-product-hero-image">
                            <img src="{{ $product['hero_image_url'] ?? asset('storage/assets/IMG_6745.JPG') }}" alt="{{ $product['name'] ?? 'Maklos Product' }}" class="h-full w-full object-cover" />
                        </div>
                        @if($product['secondary_image_url'] ?? false)
                            <div class="absolute -bottom-10 -left-10 hidden h-40 w-40 overflow-hidden rounded-3xl border border-white/60 bg-white shadow-lg shadow-maklos-100/60 sm:block">
                                <img src="{{ $product['secondary_image_url'] }}" alt="{{ $product['name'] ?? 'Maklos Product' }} detail" class="h-full w-full object-cover" />
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <section class="bg-white py-16">
                    <div class="mx-auto grid max-w-6xl gap-6 px-6 sm:grid-cols-2 lg:grid-cols-4 js-product-gallery">
                        @foreach ($gallery as $image)
                            <div class="overflow-hidden rounded-3xl border border-maklos-100 bg-white shadow-sm shadow-maklos-100/40">
                                <img src="{{ asset($image) }}" alt="{{ $product['name'] ?? 'Maklos Product' }} gallery" class="h-52 w-full object-cover" loading="lazy" />
                            </div>
                        @endforeach
                    </div>
                </section>
            ...
            <section class="bg-maklos-50 py-24 js-product-section">
                <div class="mx-auto grid max-w-6xl gap-12 px-6 lg:grid-cols-[1.2fr_1fr] lg:items-center">
                    <div class="space-y-6">
                        ...
                        <h2 class="font-display text-3xl text-charcoal sm:text-4xl">What makes {{ $product['name'] ?? 'this product' }} special</h2>
                        <p class="text-base text-charcoal/70">
                            {{ $product['description'] ?? 'Maklos products blend botanical science with meticulous production to deliver elevated care experiences for homes, hospitality, and wellness partners.' }}
                        </p>
                        <ul class="space-y-4">
                            @foreach(($product['key_points'] ?? []) as $point)
                                <li class="flex items-start gap-3 rounded-2xl border border-white/60 bg-white/80 p-4 text-charcoal/80 shadow-sm js-product-point">
                                    <span class="mt-1 inline-flex h-8 w-8 flex-none items-center justify-center rounded-full bg-maklos-500/15 text-maklos-600">
                    ...
                    @if(isset($product['sourcing']))
                        <div class="rounded-[2.5rem] border border-white bg-white/80 p-8 shadow-xl shadow-maklos-100/60">
                            ...
                            <h3 class="mt-4 font-display text-2xl text-charcoal">{{ $product['sourcing']['title'] ?? 'Crafted with care' }}</h3>
            ...
            <section class="bg-white py-24 js-product-section">
                <div class="mx-auto max-w-6xl px-6">
                    <div class="mb-10 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                        ...
                        <a href="mailto:{{ config('app.contact_email', 'info@maklostrader.com') }}" class="inline-flex items-center gap-2 rounded-full border border-maklos-200 px-6 py-3 text-sm font-semibold text-maklos-700 transition hover:border-maklos-400 hover:text-maklos-500">
                    ...
                    <div class="grid gap-6 md:grid-cols-3 js-product-availability">
                        @foreach(($product['availability'] ?? []) as $availability)
                            <article class="rounded-3xl border border-maklos-100 bg-maklos-50/40 p-6 shadow-sm shadow-maklos-100/50">
                                <h3 class="font-display text-xl text-charcoal">{{ $availability['label'] ?? 'Offering' }}</h3>
                                <p class="mt-2 text-sm text-charcoal/70">{{ $availability['description'] ?? 'Tailored solutions available upon request.' }}</p>
                            </article>
                        @endforeach
                    </div>
                </div>
                ...
                <section class="bg-white py-16">
                    <div class="mx-auto max-w-6xl px-6">
                        <div class="mb-10 flex items-center justify-between">
                            ...
                            <a href="{{ route('welcome') }}#products" class="text-sm font-semibold text-maklos-600 transition hover:text-maklos-500">Back to all products</a>
                        </div>
                        <div class="grid gap-6 md:grid-cols-3">
                            @foreach($relatedProducts as $related)
                                <a href="{{ route('products.show', $related['slug']) }}" class="group flex flex-col overflow-hidden rounded-3xl border border-maklos-100 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
                                    <div class="aspect-[4/3] overflow-hidden bg-maklos-50">
                                        <img src="{{ $related['hero_image_url'] ?? asset('storage/assets/IMG_6745.JPG') }}" alt="{{ $related['name'] ?? 'Maklos Product' }}" class="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
                                    </div>
                                    <div class="flex flex-1 flex-col p-6">
                                        <span class="inline-flex rounded-full bg-maklos-50 px-3 py-1 text-xs font-semibold uppercase tracking-[0.3em] text-maklos-600">{{ $related['category'] ?? 'Portfolio' }}</span>
                                        <h3 class="mt-4 font-display text-xl text-charcoal">{{ $related['name'] ?? 'Maklos Product' }}</h3>
                                        <p class="mt-3 flex-1 text-sm text-charcoal/70">{{ $related['excerpt'] ?? 'Discover more about this Maklos innovation.' }}</p>
