            ...
            <div class="mx-auto max-w-7xl px-6 py-28 text-center">
                <div class="space-y-8">
                    ...
                    <p class="max-w-2xl mx-auto text-lg text-charcoal/70 sm:text-xl">
                        Discover our complete collection of eucalyptus-powered products crafted with botanical science
                    ...
                    <div class="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                        @foreach($categories as $category)
                            <div class="group cursor-pointer" data-category="{{ $category['slug'] }}">
                                <div
                                    class="relative overflow-hidden rounded-3xl border border-maklos-100 bg-white shadow-sm transition-all duration-300 hover:-translate-y-2 hover:shadow-xl">
                                    ...
                                    <div class="p-6">
                                        ...
                                        <p class="mt-2 text-sm text-charcoal/70">{{ $category['products']->count() }} products
                                        </p>
        ...
        <section class="bg-maklos-50 py-24">
            <div class="mx-auto max-w-7xl px-6">
                ...
                <div class="mb-12 flex flex-wrap justify-center gap-3" id="category-filters">
                    <button
                        class="category-filter active rounded-full border border-maklos-200 bg-white px-4 py-2 text-sm font-semibold text-charcoal transition hover:bg-maklos-100"
                        data-category="all">
                        All Products
                    </button>
                    @foreach($categories as $category)
                        <button
                            class="category-filter rounded-full border border-maklos-200 bg-white px-4 py-2 text-sm font-semibold text-charcoal transition hover:bg-maklos-100"
                    ...
                    @foreach($products as $product)
                        <article class="product-card group" data-category="{{ Str::slug($product->category) }}">
                            <a href="{{ route('products.show', $product->slug) }}"
                                class="flex flex-col overflow-hidden rounded-3xl border border-maklos-100 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
                                <div class="aspect-[4/3] overflow-hidden bg-maklos-50">
                                    <img src="{{ $product->hero_image ? asset('storage/' . $product->hero_image) : asset('storage/assets/IMG_6745.JPG') }}"
                                        alt="{{ $product->name }}"
                                        class="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
                                </div>
                                <div class="flex flex-1 flex-col p-6">
                                    <span
                                        class="inline-flex rounded-full bg-maklos-50 px-3 py-1 text-xs font-semibold uppercase tracking-[0.3em] text-maklos-600">{{ $product->category }}</span>
                                    <h3 class="mt-4 font-display text-xl text-charcoal">{{ $product->name }}</h3>
                                    <p class="mt-3 flex-1 text-sm text-charcoal/70">{{ $product->excerpt }}</p>
                                    <div class="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-maklos-600">
                                        View details
                                        <svg xmlns="http://www.w3.org/2000/svg"
                                            class="h-4 w-4 transition group-hover:translate-x-1" fill="none"
