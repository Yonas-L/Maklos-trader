...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="bg-white font-sans text-charcoal antialiased">
        @php
            $heroSlides = collect(\Illuminate\Support\Facades\Storage::disk('public')->files('assets'))
                ->filter(fn ($path) => preg_match('/\.(jpe?g|png|webp|avif|heic)$/i', $path))
                ->map(function ($path) {
                    $filename = pathinfo($path, PATHINFO_FILENAME);
                    $title = \Illuminate\Support\Str::of($filename)
                        ->replace(['_', '-'], ' ')
                        ->headline();
                    ...
                    return [
                        ...
                        'title' => $title,
                        'caption' => 'Premium Maklos formulation crafted with natural botanicals.',
                    ];
                })
                ->values();
            ...
            if ($heroSlides->isEmpty()) {
                $heroSlides = collect([
                    [
                        'image' => asset('storage/assets/IMG_6745.JPG'),
                        'title' => 'Eucalyptus Laundry Soap',
                        'caption' => 'Powerful stain removal with the calming scent of eucalyptus and tea tree oils.',
                    ],
                ]);
            }

            $productImages = $heroSlides->pluck('image')->all();
